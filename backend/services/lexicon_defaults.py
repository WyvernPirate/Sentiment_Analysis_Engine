"""Default seed content for the Setswana lexicon.

Kept as a plain data module (no DB/service dependencies) so it can be
imported both by the DB seeding step (models.py) and by anything that
wants the baseline word list without touching the database.
"""
